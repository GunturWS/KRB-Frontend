  ...
  return (
    <header className="sticky top-0 left-0 right-0 z-50 w-full bg-brand-secondary-07/90 backdrop-blur-md shadow-sm">
      <div className="flex w-full items-center justify-between p-4 sm:container mx-auto">
        ...
        {/* Menu */}
        <div
          className={`header__menu fixed inset-x-0 top-24 mx-auto w-[85%] origin-top rounded-3xl bg-gray-900 p-8 text-center transition-all duration-300 
            md:static md:inset-auto md:top-0 md:m-0 md:flex md:w-auto md:scale-y-100 md:items-center md:gap-12 md:bg-transparent md:p-0 md:opacity-100 
        ...
        >
            ...
            {navLinks.map(({ to, label }) => (
              <li key={to}>
                <NavLink
                  ...
                  className={({ isActive }) =>
                    isActive
                      ? "text-[#629f80] font-semibold border-b-2 border-emerald-500"
          ...
          {/* Tombol Dashboard */}
          <Link to="/admin/login">
            <button className="mt-6 md:mt-0 px-6 py-2 bg-gradient-to-r from-[#629f80] to-emerald-500 text-white rounded-xl font-semibold shadow hover:opacity-90 transition">
              Dashboard
            </button>
          </Link>
        </div>
